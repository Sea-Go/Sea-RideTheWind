...
本分支验收实测：上述完整命令以 `-race` 运行通过；`TestRealHTTPKnowledgeWorkflowWithUserCenter`、普通 go-zero/PG HTTP 流程、`TestProductSearchOperationFixedSnapshotLeaseAndReplay` 与 `TestProductSearchMigrationProbeAndBadKeys` 均 PASS，脚本内 `go vet ./service/knowledge/...` 和 User Center race/vet 也通过。成功、202 与 503 均按真实 HTTP 状态断言；测试在 BTW fixture 返回伪 200 而未 Commit 时得到 503、历史为空，在 BTW 通过真实 RTW 私有 HTTP Commit 后故意返回 502 时仍按 AnswerID 恢复 200。操作表迁移脚本在旧结构（缺表）隔离 schema 上实跑并通过启动前探测。正式 BTW 进程、真实三路索引、模型总结、网页/桌宠和 Collector 下钻尚未在本分支验收。
