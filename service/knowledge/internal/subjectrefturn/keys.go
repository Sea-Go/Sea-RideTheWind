// Package subjectrefturn scans frozen v1 root-turn JSON for ambiguous keys.
// Its critical-path mode is shared by the stage-1 preflight and v2 reads.
...
// ScanOptions makes the v2 product read stricter for exact duplicate keys,
// while the stage-1 preflight retains its documented critical-path behavior.
type ScanOptions struct {
	RejectAnyExactDuplicate bool
}

func AmbiguousFrozenTurnKeys(raw []byte, options ScanOptions) (bool, error) {
	decoder := json.NewDecoder(bytes.NewReader(raw))
	ambiguous, err := scanTurnValue(decoder, turnRoot, 0, options)
	if err != nil {
		return false, err
	}
...
func scanTurnValue(decoder *json.Decoder, object turnObject, depth int, options ScanOptions) (bool, error) {
	...
	case '{':
		...
		for decoder.More() {
			...
			if options.RejectAnyExactDuplicate && seenExact[key] {
				ambiguous = true
			}
			seenExact[key] = true
			...
			childAmbiguous, err := scanTurnValue(decoder, child, depth+1, options)
			if err != nil {
				return false, err
			}
	...
	case '[':
		ambiguous := false
		for decoder.More() {
			childAmbiguous, err := scanTurnValue(decoder, object, depth+1, options)
			if err != nil {
				return false, err
			}
			ambiguous = ambiguous || childAmbiguous
