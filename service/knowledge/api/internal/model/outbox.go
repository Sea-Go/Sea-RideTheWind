...
func (s *Store) DispatchOne(ctx context.Context, sender Sender) (bool, error) {
	...
	var raw []byte
	err = tx.QueryRow(ctx, "SELECT payload FROM knowledge_outbox WHERE delivered_at IS NULL ORDER BY created_at,event_id FOR UPDATE SKIP LOCKED LIMIT 1").Scan(&raw)
