...
-- One immutable human Source Fact judgment belongs to one immutable Wiki
-- revision, not to the current editing head or manually published Release.
-- Rejudging advances only the (wiki_revision_id,fact_id) judgment head.
...
-- The original Knowledge EventSpec and correlation remain frozen. Delivery
-- may fill delivered_at once; it cannot rewrite this human label's source.
...
-- A human declaration lists all expected Source Facts for exactly one
-- approved page Source scope and immutable Wiki revision. This head is
-- independent of Wiki editing heads, quality judgment heads and Release.
...
-- DC delivery may fill delivered_at once; the EventSpec identity and
-- correlation of a frozen FactSet cannot be rebuilt from outbox JSONB.
