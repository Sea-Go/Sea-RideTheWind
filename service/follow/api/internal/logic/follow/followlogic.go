...
func (l *FollowLogic) Follow(req *types.RelationActionReq) (resp *types.RelationActionResp, err error) {
	const route = "/follow/v1/follow"
	started := time.Now()
	defer func() {
	...
	rpcResp, rpcErr := l.svcCtx.FollowRpc.Follow(l.ctx, &followservice.RelationReq{
		UserId:   userID,
