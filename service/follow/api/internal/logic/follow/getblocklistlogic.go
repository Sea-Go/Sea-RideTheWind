...
func (l *GetBlockListLogic) GetBlockList(req *types.FollowListReq) (resp *types.FollowUserListResp, err error) {
	const route = "/follow/v1/block/list"
	started := time.Now()
	defer func() {
	...
	rpcResp, rpcErr := l.svcCtx.FollowRpc.GetBlockList(l.ctx, &followservice.ListReq{
		UserId: queryUserID,
		Offset: req.Offset,
		Limit:  req.Limit,
	})
